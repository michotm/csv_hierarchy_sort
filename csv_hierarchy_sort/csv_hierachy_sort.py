...
def compute_complete_path(
    indexed_csv, current_id, complete_path, parent_id_key="parent_id"
):
    if (
        current_id in indexed_csv
        and indexed_csv[current_id][parent_id_key]
        and indexed_csv[current_id][parent_id_key] in indexed_csv
    ):
        return compute_complete_path(
            indexed_csv,
            indexed_csv[current_id][parent_id_key],
            f"{indexed_csv[current_id][parent_id_key]}/{complete_path}",
            parent_id_key,
        )
    else:
...
def main(id_key, parent_id_key, input_file, output_file, delimiter):
    ...
    l = [
        indexed_csv[r]["complete_path"].split("/")
        for r in indexed_csv
        if indexed_csv[r]["complete_path"] != ""
    ]
    levels = {}
